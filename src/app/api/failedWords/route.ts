...
export async function GET() {
  try {
    const words = await query<faileWords[]>(
      "SELECT kr,en,kakaoid FROM falsehangul",
    );
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!Array.isArray(words) || words.length === 0) {
      ...
      return NextResponse.json(
        { error: "유효한 단어 목록이 필요합니다." },
        { status: 400 },
      );
    }

    const values = words.map(({ korean, english }) => [
      korean,
      english,
      kakaoid,
    ]);
    ...
    await query(
      `INSERT INTO falsehangul (kr, en, kakaoid) VALUES ${placeholders}`,
      flatValues,
    );
