...
export default function WordGame() {
  ...
  const [correctWords, setCorrectWords] = useState<{ x: number; y: number }[]>([]); // ✅ 정답 위치 상태
  ...
  const handleCheckWord = () => {
    ...
    if (matchedWord) {
      ...
      setCorrectWords((prev) => [...prev, { x: matchedWord.x, y: matchedWord.y }]);

      setTimeout(() => {
        // 1초 후 정답 효과 제거
        setCorrectWords((prev) =>
          prev.filter((pos) => pos.x !== matchedWord.x || pos.y !== matchedWord.y)
        );
